...
const JobDetailPageCompany = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredApps = appsData.filter(
        (app) => Number(app.job_id) === Number(id) && app.status !== "rejected",
      );
      ...
      setFormData({
        ...
        skils: Array.isArray(jobData.skils)
          ? jobData.skils.join(", ")
          : jobData.skils || "",
  ...
  const openInviteModal = (talent) => {
    ...
    setInviteData({
      ...inviteData,
      message: `A new ${job.occupation} position matches your skills at ${job.company?.company_name || "our company"}.`,
    });
    setIsInviteModalOpen(true);
  };

  const handleConfirmInvitation = async () => {
    const loadingToast = toast.loading("Taklif yuborilmoqda...");
    try {
      await notificationApi.send({
        talent_id: Number(selectedTalent.id),
        title: String(inviteData.title),
        message: String(inviteData.message),
        type: "job",
      });

      const data = new FormData();
      data.append("job_id", Number(id));
      data.append("talent_id", Number(selectedTalent.id));
      data.append("status", "pending");

      await applicationApi.apply(data);

      toast.success("Taklif muvaffaqiyatli yuborildi!", { id: loadingToast });
      setIsInviteModalOpen(false);
      fetchData();
    } catch (err) {
      console.error("Xato tafsiloti:", err.response?.data || err);
      const errorMsg =
        err.response?.data?.message || "Yuborishda xatolik yuz berdi";
  ...
  const handleUpdateJob = async (e) => {
    ...
    toast.promise(updatePromise, {
      ...
      error: (err) =>
        `Xatolik: ${err.response?.data?.message || "Saqlab bo'lmadi"}`,
    });
              ...
              <div className="text-center">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-3.5 bg-red-500 text-white font-bold rounded-2xl"
                  >
                    O'chirish
                  </button>
                </div>
        ...
        <div className="pb-20">
          ...
          <div className="grid grid-cols-1 min-[1350px]:grid-cols-2 gap-6 sm:gap-8 w-full">
            {(activeDetailTab === "Matches"
              ...
              const talent =
                activeDetailTab === "Matches" ? item : item.talent || {};
              const appId = item.id;
              ...
              return (
                talent && (
                  <div
                    key={uniqueKey}
                    className="border rounded-[2rem] shadow-sm p-6 sm:p-7 flex flex-col justify-between hover:shadow-md transition-all min-h-[400px] bg-white border-gray-100"
                  >
                    <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-5 mb-5 w-full">
                      <div className="flex flex-col lg:flex-row items-center gap-4 flex-1 w-full min-w-0">
                        <div className="w-20 h-20 sm:w-24 sm:h-24 shrink-0 rounded-full flex items-center justify-center overflow-hidden border bg-[#F3F4F6] border-gray-100">
                          {talent.image ? (
                            <img
                              src={talent.image}
                              className="w-full h-full object-cover"
                        ...
                        <div className="w-full min-w-0 text-center lg:text-left">
                          <h2 className="text-[20px] sm:text-[24px] font-bold leading-tight mb-1 text-[#3a3a3a]">
                            {talent.specialty || "Specialist"}
                          </h2>
                          <p className="text-gray-500 text-[16px] sm:text-[18px] font-medium">
                            {talent.first_name} {talent.last_name}
                          </p>
                        </div>
                      </div>
                      <div className="shrink-0 w-full lg:w-auto border-t lg:border-none pt-4 lg:pt-0 text-center lg:text-right border-gray-100">
                        <div className="text-[22px] sm:text-[26px] font-bold leading-tight text-[#1D3D54]">
                    ...
                    <div className="flex-1 my-6">
                      ...
                      <div className="flex flex-wrap justify-center lg:justify-start gap-2">
                        {parseTalentSkills(talent.skils)
                          .slice(0, 6)
                          .map((s, i) => (
                            <span
                              key={i}
                              className="px-4 py-1.5 text-[13px] font-semibold rounded-xl border bg-[#f8fafc] border-gray-100 text-[#64748b]"
                            >
                              {s.skill}
                            </span>
                          ))}
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mt-auto items-center justify-center w-full">
                      <button
                        ...
                        className="flex-1 w-full py-3 px-4 border-2 rounded-2xl font-bold transition-all duration-300 shadow-sm text-sm sm:text-base border-[#1D3D54] text-[#1D3D54] hover:bg-[#1D3D54] hover:text-white"
                      >
                        View Profile
                      </button>
                      ...
                      {activeDetailTab === "Matches" ? (
                        <button
                          ...
                          className="flex-1 w-full py-3 px-4 rounded-2xl font-bold transition-all shadow-md active:scale-95 text-sm sm:text-base flex items-center justify-center gap-2 bg-[#1D3D54] text-white hover:bg-[#2c5a7a]"
                        >
                          <FiSend size={16} /> Send Invitation
                        </button>
                      ) : (
                        <div className="flex-1 w-full flex items-center justify-center gap-3">
                          {status === "pending" ? (
                            <>
                              {resumeUrl && (
                                <a
                                  ...
                                  className="flex-1 h-[52px] flex items-center justify-center gap-2 rounded-2xl font-bold transition-all border shadow-sm text-sm bg-blue-50 text-blue-600 border-blue-100 hover:bg-blue-600 hover:text-white"
                                >
                              ...
                              <button
                                ...
                                className="h-12 w-12 shrink-0 rounded-full flex items-center justify-center transition-all duration-300 shadow-sm border group bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-500 hover:text-white"
                              >
                                <MdCheck
                                  size={28}
                              ...
                              <button
                                ...
                                className="h-12 w-12 shrink-0 rounded-full flex items-center justify-center transition-all duration-300 shadow-sm border group bg-rose-50 text-rose-600 border-rose-100 hover:bg-rose-500 hover:text-white"
                              >
                                <MdClose
                                  size={28}
                                  className="group-hover:scale-110 transition-transform"
                            ...
                            <div
                              className={`flex-1 py-3 text-center rounded-2xl font-bold uppercase text-xs tracking-widest border-2 ${status === "accepted" ? "border-emerald-500 bg-emerald-50/10 text-emerald-500" : "border-rose-500 bg-rose-50/10 text-rose-500"}`}
                            >
                              {status}
                            </div>
                          )}
